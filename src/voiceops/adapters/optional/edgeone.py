...
from .edgeone_witness import (
    is_remote_edgeone_witness_url,
    pages_project,
    verify_witness_post,
    witness_url,
)
...
def status() -> AdapterStatus:
    ...
    if url and is_remote_edgeone_witness_url(url):
        ...
        if verify.get("ok"):
            return AdapterStatus(
                ...
                evidence_note="Remote EdgeOne witness accepted sanitized execution receipt",
                extra={
        ...
        return AdapterStatus(
            ...
            mode="witness",
            evidence_note="Remote EdgeOne witness URL configured but POST probe failed",
            extra={"pages_project": project or None, "witness_url": url, "verify": verify},
        )

    if url and not is_remote_edgeone_witness_url(url):
        return AdapterStatus(
            adapter_id="edgeone",
            provider="Tencent EdgeOne",
            truth="CONFIGURED",
            status="CONFIGURED",
            mode="witness_local_smoke",
            ready=False,
            remote_confirmed=False,
            evidence_note=(
                "Local witness smoke URL ignored for REAL — deploy agents/execution-witness on EdgeOne"
            ),
            extra={
                "pages_project": project or None,
                "local_smoke_url": url,
